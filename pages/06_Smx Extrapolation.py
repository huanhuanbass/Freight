...
st.title('Supramax S10TC Extrapolation')
...
spotsmx=spot[['S10TC']]
spotsmx.sort_index(inplace=True)
...
ldffa=currmffa['10TC_SCURMON'][-1]

st.markdown('## **Current Month Extrapolation**')
st.markdown('#### **----Extrapolation from FFA to Spot**')

spotsmx['Year']=spotsmx.index.year
spotsmx['Quarter']=spotsmx.index.quarter
spotsmx['Month']=spotsmx.index.month

mtd=spotsmx[(spotsmx['Year']==curryear) & (spotsmx['Month']==currmonth)]
mtdavg=mtd['S10TC'].mean()
ldspot=spotsmx.iloc[-1,0]
...
mrest=pd.DataFrame(data={'Date':futday,'Implied S10TC Average':extpl})
...
mchart=m[['S10TC','Current Month FFA Input','Implied S10TC Average','Simulation']]

lplot=px.line(mchart,width=1000,height=500,title='S10TC Current Month Extrapolation: From FFA to Spot')
...
mchart2=m2[['S10TC','Estimated Spot Left','Implied FFA','Simulation']]

lplot2=px.line(mchart2,width=1000,height=500,title='S10TC Current Month Extrapolation: From Spot to FFA')
